...
PASSKIT_API_KEY = "0anQzO4Pfe3u6L6Xvr2p0tDUgKmAB5W2Pmw7jg6LOCjfVZVdnOIiCmQ5d39ZfrmZTwLniqzdl_5lHXJ5Q3ZV7Liu4-NAwsDktCszjE8GtnHQKVQm6CDKK485DnqQT7ou6Q4haj-Qi90hxf3Q8Vg9PssaVKBlAilP8Ff581of7mswTYWY4YcRu2SllDfx3wQyXlTJf1xMe14TnVnQJaYS2TNUbj4AAVjDmweqPMCrfE6N7E2h3T7SsGrQGf43tTJiT1oQ0PAW-Qwr4OZgrRnblI9zuGa78YSmFWyShKSflIcPTES-sP8b6VCKPHv4olRM"
...
// --- CONFIGURATION ---
const PASSKIT_API_KEY = "{PASSKIT_API_KEY}";
const PASSKIT_PROGRAM_ID = "REPLACE_WITH_YOUR_PROGRAM_ID"; // <--- PASTE YOUR PASSKIT PROGRAM ID HERE
    ...
    const emailBody = `
      <div style="font-family: 'Courier New', monospace; background-color: #050f0d; color: white; padding: 40px; text-align: center;">
        ...
        <div style="margin-top: 20px;">
          <a href="${{passUrl}}" style="background-color: #000000; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; border: 1px solid #333; display: inline-block;">
            Add to Apple Wallet
          </a>
        </div>
...
function createAppleWalletPass(data) {{
  if (PASSKIT_PROGRAM_ID === "REPLACE_WITH_YOUR_PROGRAM_ID") {{
    console.log("PassKit Program ID missing.");
    return "https://wallet.apple.com/";
  }}

  // Endpoint for creating a member (which generates a pass)
  const url = "https://api.pub1.passkit.io/members";
  
  const payload = {{
    "programId": PASSKIT_PROGRAM_ID,
    "externalId": data.email, // Use email as unique ID
    "metaData": {{
      "Name": data.name,
  ...
  try {{
    ...
    if (responseCode === 200 || responseCode === 201) {{
       // Return the landing page URL where the user can add the pass
       return json.landingPageUrl || "https://wallet.apple.com/";
    }} else {{
       console.log("PassKit API Error: " + JSON.stringify(json));
    }}
  }} catch (e) {{
    console.log("PassKit Exception: " + e.toString());
  }}
...
print("Updated script with PassKit integration.")
